...
// 如果传入 language 为空，则从配置文件读取默认语言
func NewGenerator(language ...string) *Generator {
	lang := ""
	if len(language) > 0 && language[0] != "" {
		lang = language[0]
	} else {
		lang = config.GetString("app.language", "zh") // 默认为中文
	}
	return &Generator{Language: lang}
}

// IsEnglish 判断是否为英文模式
func (p *Generator) IsEnglish() bool {
	return p.Language == "en"
}

// GetStoryboardSystemPrompt 获取分镜生成系统提示词
func (p *Generator) GetStoryboardSystemPrompt() string {
	if p.IsEnglish() {
...
func (p *Generator) GetSceneExtractionPrompt(style string) string {
	imageRatio := "16:9"
...
func (p *Generator) GetFirstFramePrompt(style string) string {
	imageRatio := "16:9"
	if p.IsEnglish() {
...
func (p *Generator) GetKeyFramePrompt(style string) string {
	imageRatio := "16:9"
	if p.IsEnglish() {
		return fmt.Sprintf(`You are a professional image generation prompt expert. Please generate prompts suitable for AI image generation based on the provided shot information.
...
func (p *Generator) GetActionSequenceFramePrompt(style string) string {
	imageRatio := "16:9"
	if p.IsEnglish() {
...
func (p *Generator) GetLastFramePrompt(style string) string {
	imageRatio := "16:9"
	if p.IsEnglish() {
		return fmt.Sprintf(`You are a professional image generation prompt expert. Please generate prompts suitable for AI image generation based on the provided shot information.
...
func (p *Generator) GetOutlineGenerationPrompt() string {
	if p.IsEnglish() {
...
func (p *Generator) GetCharacterExtractionPrompt(style string) string {
	imageRatio := "16:9"
	if p.IsEnglish() {
...
func (p *Generator) GetPropExtractionPrompt(style string) string {
	imageRatio := "1:1"
...
func (p *Generator) GetEpisodeScriptPrompt() string {
	if p.IsEnglish() {
...
func (p *Generator) FormatUserPrompt(key string, args ...interface{}) string {
...
func (p *Generator) GetStylePrompt(style string) string {
	if style == "" {
		return ""
	}
...
- **光影表现手法**：强调**"弥散的点光源（Diffuse Point Lights）"**。光线不是硬性的投射，而是呈晕染状。例如，路灯、车灯或月亮周围有一圈柔和朦胧光晕（Glow effect）。地面通常带有微弱的雨后反光或湿润感，增加光影的层次感和梦幻感。`,
...
func (p *Generator) GetVideoConstraintPrompt(referenceMode string) string {
	// 动作序列图（九宫格）的约束提示词
