<template>
  <div>
    <template v-for="item in list" :key="item.path">
      ...
      <t-submenu v-else :name="item.path" :value="item.path" :title="renderMenuTitle(item.title)" style="padding: 5px 10px 5px 16px;">
        <template #icon>
          <component :is="menuIcon(item)" class="t-icon" style="height: 15px;width:15px"></component>
        </template>
