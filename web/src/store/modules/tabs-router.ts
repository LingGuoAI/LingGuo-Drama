...
export const useTabsRouterStore = defineStore("tabsRouter", {
  ...
  actions: {
    ...
    subtractTabRouterBehind(newRoute: TRouterInfo) {
      const { routeIdx } = newRoute;
      const homeIdx: number = this.tabRouters.findIndex(
        (route: TRouterInfo) => route.isHome,
      );
      let tabRouterList: Array<TRouterInfo> = this.tabRouterList.slice(
        0,
        routeIdx + 1,
      );
      if (routeIdx < homeIdx) {
    ...
    subtractTabRouterAhead(newRoute: TRouterInfo) {
      ...
      let tabRouterList: Array<TRouterInfo> =
        this.tabRouterList.slice(routeIdx);
      if (routeIdx > homeIdx) {
