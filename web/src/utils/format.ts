...
// 字典过滤
export const filterDict = (value: any, options: Array<{ label: string; value: any }>): string => {
  const option = options.find(item => item.value === value)
  return option ? option.label : String(value)
}

// 数据源过滤
export const filterDataSource = (dataSource: Array<{ label: string; value: any }>, value: any): string | string[] => {
...
// 获取字典数据的模拟函数
export const getDictFunc = async (dictType: string): Promise<Array<{ label: string; value: any }>> => {
  // 这里应该调用实际的字典API
